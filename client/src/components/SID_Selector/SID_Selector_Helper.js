const removeAgreementSidsFromOppySids = (
	oppySidRecords,
	agreementSIDRecords
) => {
	let agreementSID_IDArray = agreementSIDRecords.map(
		(sid) => sid.Account_SID__r.Id
	);
...
const sortAgreementSIDs = (agreementSIDs) => {
	let primaryActionSIDs = agreementSIDs.filter(
		(sid) => sid.Is_Primary_Account_SID__c
	);
	let flexActionSIDs = agreementSIDs.filter(
